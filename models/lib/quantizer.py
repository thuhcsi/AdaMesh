...
class VectorQuantizer(nn.Module):
    ...
    def forward(self, z):
        ...
        z_flattened = z.view(-1, self.e_dim)  # [1*T*16, 64]

        # distances from z to embeddings
        d = torch.sum(z_flattened ** 2, dim=1, keepdim=True) + \
            torch.sum(self.embedding.weight**2, dim=1) - 2 * \
            torch.matmul(z_flattened, self.embedding.weight.t())  # [1*T*16, 256]
        ...
        min_encoding_indices = torch.argmin(d, dim=1).unsqueeze(1)  # [1*T*16, 1]
        min_encodings = torch.zeros(min_encoding_indices.shape[0], self.n_e).to(z) # [1*T*16, 256] 
        min_encodings.scatter_(1, min_encoding_indices, 1)


        # get quantized latent vectors
        z_q = torch.matmul(min_encodings, self.embedding.weight).view(z.shape) # [1, T*16, 64]
