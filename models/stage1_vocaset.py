...
class VQAutoEncoder(BaseModel):
    ...
    def encode(self, x, x_a=None):
        h = self.encoder(x) ## x --> z' [1, T, 1024]
        # import pdb; pdb.set_trace()
        h = h.view(x.shape[0], -1, self.args.face_quan_num, self.args.zquant_dim) # [1, T, 16, 64]
        h = h.view(x.shape[0], -1, self.args.zquant_dim) # [1, T*16, 64]
    ...
    def decode(self, quant):
        #BCL
        quant = quant.permute(0,2,1)  # [1, T*16, 64]
        quant = quant.view(quant.shape[0], -1, self.args.face_quan_num, self.args.zquant_dim).contiguous() # [1, T, 16, 64]
        quant = quant.view(quant.shape[0], -1,  self.args.face_quan_num*self.args.zquant_dim).contiguous() # [1, T, 1024]
        quant = quant.permute(0,2,1).contiguous() # [1, 1024, T]
