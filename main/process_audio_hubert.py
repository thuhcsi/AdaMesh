...
hubert_local_path = "/cfs2/user/chenliyang/Project/ArtFace/hubert-large-ls960-ft"
print("Loading the Wav2Vec2 Processor...")
# wav2vec2_processor = Wav2Vec2Processor.from_pretrained("facebook/hubert-large-ls960-ft")
wav2vec2_processor = Wav2Vec2Processor.from_pretrained(hubert_local_path)
print("Loading the HuBERT Model...")
# hubert_model = HubertModel.from_pretrained("facebook/hubert-large-ls960-ft")
hubert_model = HubertModel.from_pretrained(hubert_local_path)
...
if __name__ == '__main__':
    ...
    import glob, os, tqdm
    lrs3_dir = "/cfs2/user/chenliyang/Data/MEAD_processed/wav"
    wav_16k_names = glob.glob(os.path.join(lrs3_dir, '**/*.wav'), recursive=True)
    for wav_16k_name in tqdm.tqdm(wav_16k_names, total=len(wav_16k_names)):
        clip_id = wav_16k_name.split("/")[-1][:-4]
        out_name = os.path.join(lrs3_dir, clip_id+'_hubert.npy')
